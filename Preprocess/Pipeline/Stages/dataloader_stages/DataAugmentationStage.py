...
class DataAugmentationStage(PipelineStage):
    ...
    def __init__(self,
                 ...
                 landmark_flip_map: Optional[Dict[int, int]] = None,  # e.g. {left_eye_idx: right_eye_idx, ...}
                 verbose: bool = False):
        """
    ...
    def process(self, data: torch.Tensor, verbose: bool = True) -> torch.Tensor:
        ...
        Args:
            data (torch.Tensor): Landmark tensor of shape (T, N, C).
                                 Expected C >= 2 (for X, Y coordinates).
            verbose (bool): If True, prints detailed status messages for this specific call.

        Returns:
            torch.Tensor: Augmented landmark tensor.
        """
        if not isinstance(data, torch.Tensor):
            if self.verbose_stage or verbose: print("DataAugmentationStage: Input is not a tensor. Skipping.")
            return data
        if data.ndim != 3 or data.shape[2] < 2:  # Need at least X, Y
            if self.verbose_stage or verbose: print(
                f"DataAugmentationStage: Invalid tensor shape {data.shape}. Expected 3D with C>=2. Skipping.")
            return data

        if self.verbose_stage or verbose: print(f"DataAugmentationStage: Processing tensor of shape {data.shape}")

        augmented_frames = []
        num_landmarks = data.shape[1]

        for frame_idx in range(data.shape[0]):
            current_landmarks = data[frame_idx, :, :].clone()  # Shape (N, C)

            # --- Horizontal Flip ---
            if random.random() < self.random_flip_prob and self.landmark_flip_map:
                if self.verbose_stage or verbose: print(f"  Frame {frame_idx}: Applying Horizontal Flip")
            ...
            if random.random() < self.random_rotate_prob:
                ...
                if self.verbose_stage or verbose: print(
                    f"  Frame {frame_idx}: Applying Rotation (angle: {math.degrees(angle):.2f} deg)")
            ...
            if random.random() < self.random_scale_prob:
                ...
                if self.verbose_stage or verbose: print(
                    f"  Frame {frame_idx}: Applying Scale (factor: {scale_factor:.2f})")
            ...
            if random.random() < self.add_noise_prob:
                if self.verbose_stage or verbose: print(f"  Frame {frame_idx}: Adding Noise (std: {self.noise_std})")
        ...
        if not augmented_frames:
            if self.verbose_stage or verbose: print(
                "DataAugmentationStage: No frames processed. Returning original data.")
            return data

        output_tensor = torch.stack(augmented_frames, dim=0)
        if self.verbose_stage or verbose: print(
            f"DataAugmentationStage: Finished processing. Output shape: {output_tensor.shape}")
