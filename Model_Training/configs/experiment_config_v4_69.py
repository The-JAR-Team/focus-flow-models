# engagement_hf_trainer/configs/experiment_config_v4_69.py
...
# Focus: Strongly boost "Barely Engaged"
CLASSIFICATION_CLASS_WEIGHTS = torch.tensor([1.0, 2.5, 1.0, 1.5, 0.8], dtype=torch.float32)
CLASSIFICATION_LOSS_FN = nn.CrossEntropyLoss(weight=CLASSIFICATION_CLASS_WEIGHTS)

# --- Paths and Naming ---
BASE_OUTPUT_DIR = "./training_runs_output/"
EXPERIMENT_NAME = "multitask_v4_69" # Focus: No Temporal Jitter, Stronger Static Augs, Adjust Class Weights for "Barely Engaged"

# --- Data Augmentation Parameters ---
MESH_FLIP_MAP = mesh_annotations_derived_flip_map
# Focus: No Temporal Jitter, Stronger Static Augs
DATA_AUGMENTATION_PARAMS = {
    "add_noise_prob": 0.7,  # Stronger
    "noise_std": 0.006,  # Stronger
    "random_scale_prob": 0.6,  # Stronger
    "scale_range": (0.90, 1.10),  # Wider range
    "random_rotate_prob": 0.6,  # Stronger
    "max_rotation_angle_deg": 18.0,  # Wider range
    "random_flip_prob": 0.5,  # Stronger
    "landmark_flip_map": MESH_FLIP_MAP,
    "temporal_jitter_prob": 0.0,  # Disabled
    "jitter_burst_length_range": (3, 10), # Default, but not used
    "jitter_magnitude_std": 0.05, # Default, but not used
    "max_jitter_bursts_per_sequence": 0, # Disabled
    "verbose": False
}
