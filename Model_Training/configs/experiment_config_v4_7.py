# engagement_hf_trainer/configs/experiment_config_v4_7.py
...
# DistanceNormalizationStage is intentionally not imported for this experiment
...
MODEL_PARAMS = {
    "hidden_dim": 256,
    "num_gru_layers": 2,
    "dropout_rate": 0.20,  # Lowered, as less normalization might need less aggressive regularization
...
CLASSIFICATION_CLASS_WEIGHTS = torch.tensor([1.0, 1.2, 1.5, 1.8, 0.8], dtype=torch.float32) # Standard v4_5 weights
CLASSIFICATION_LOSS_FN = nn.CrossEntropyLoss(weight=CLASSIFICATION_CLASS_WEIGHTS)

# --- Paths and Naming ---
BASE_OUTPUT_DIR = "./training_runs_output/"
EXPERIMENT_NAME = "multitask_v4_7" # Focus: No Distance Normalization, Lower Data Augmentation

# --- Data Augmentation Parameters ---
MESH_FLIP_MAP = mesh_annotations_derived_flip_map
# Focus: Significantly reduced due to no distance norm
DATA_AUGMENTATION_PARAMS = {
    "add_noise_prob": 0.2,
    "noise_std": 0.001,
    "random_scale_prob": 0.2, # Scale might be tricky without normalization, keep low
    "scale_range": (0.98, 1.02),
    "random_rotate_prob": 0.2,
    "max_rotation_angle_deg": 5.0,
    "random_flip_prob": 0.1,
    "landmark_flip_map": MESH_FLIP_MAP,
    "temporal_jitter_prob": 0.1,
    "jitter_burst_length_range": (2, 5),
    "jitter_magnitude_std": 0.02,
    "max_jitter_bursts_per_sequence": 1,
    "verbose": False
}
...
# distance_normalization_stage_instance is NOT instantiated or used in pipelines for this experiment.
...
# Distance Normalization Stage is removed from all pipelines
TRAIN_PIPELINE = OrchestrationPipeline(stages=[
    label_processor_stage_instance,
    # distance_normalization_stage_instance, # REMOVED
